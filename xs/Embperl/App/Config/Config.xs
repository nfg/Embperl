...
void Embperl__App__Config_new_init (pTHX_ Embperl__App__Config  obj, SV * item, int overwrite) {
    ...
    if (SvTYPE(item) == SVt_PVMG) 
        ...
        if ((tmpsv = hv_fetch((HV *)item, "app_name", sizeof("app_name") - 1, 0)) || overwrite)
            obj -> sAppName = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "app_handler_class", sizeof("app_handler_class") - 1, 0)) || overwrite)
            obj -> sAppHandlerClass = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "session_handler_class", sizeof("session_handler_class") - 1, 0)) || overwrite)
            obj -> sSessionHandlerClass = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "session_args", sizeof("session_args") - 1, 0)) || overwrite)
            obj -> pSessionArgs = (HV *)SvREFCNT_inc(((HV *)epxs_sv2_HVREF((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "session_classes", sizeof("session_classes") - 1, 0)) || overwrite)
            obj -> pSessionClasses = (AV *)SvREFCNT_inc(((AV *)epxs_sv2_AVREF((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "session_config", sizeof("session_config") - 1, 0)) || overwrite)
            obj -> sSessionConfig = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "cookie_name", sizeof("cookie_name") - 1, 0)) || overwrite)
            obj -> sCookieName = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "cookie_domain", sizeof("cookie_domain") - 1, 0)) || overwrite)
            obj -> sCookieDomain = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "cookie_path", sizeof("cookie_path") - 1, 0)) || overwrite)
            obj -> sCookiePath = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "cookie_expires", sizeof("cookie_expires") - 1, 0)) || overwrite)
            obj -> sCookieExpires = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "log", sizeof("log") - 1, 0)) || overwrite)
            obj -> sLog = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        ...
        if ((tmpsv = hv_fetch((HV *)item, "mailhost", sizeof("mailhost") - 1, 0)) || overwrite)
            obj -> sMailhost = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "mailhelo", sizeof("mailhelo") - 1, 0)) || overwrite)
            obj -> sMailhelo = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "mailfrom", sizeof("mailfrom") - 1, 0)) || overwrite)
            obj -> sMailfrom = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        ...
        if ((tmpsv = hv_fetch((HV *)item, "mail_errors_to", sizeof("mail_errors_to") - 1, 0)) || overwrite)
            obj -> sMailErrorsTo = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        ...
        if ((tmpsv = hv_fetch((HV *)item, "object_base", sizeof("object_base") - 1, 0)) || overwrite)
            obj -> sObjectBase = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "object_app", sizeof("object_app") - 1, 0)) || overwrite)
            obj -> sObjectApp = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "object_addpath", sizeof("object_addpath") - 1, 0)) || overwrite)
            obj -> pObjectAddpathAV = (AV *)SvREFCNT_inc(((AV *)epxs_sv2_AVREF((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "object_stopdir", sizeof("object_stopdir") - 1, 0)) || overwrite)
            obj -> sObjectStopdir = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "object_fallback", sizeof("object_fallback") - 1, 0)) || overwrite)
            obj -> sObjectFallback = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "object_handler_class", sizeof("object_handler_class") - 1, 0)) || overwrite)
            obj -> sObjectHandlerClass = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
   ; }
...
CODE:
    ...
    if (initializer) {
        ...
        if (SvTYPE(tmpsv) == SVt_PVHV || SvTYPE(tmpsv) == SVt_PVMG)  
            Embperl__App__Config_new_init (aTHX_ cobj, tmpsv, 1) ;
