...
void Embperl__Req__Param_new_init (pTHX_ Embperl__Req__Param  obj, SV * item, int overwrite) {
    ...
    if (SvTYPE(item) == SVt_PVMG) 
        ...
        if ((tmpsv = hv_fetch((HV *)item, "filename", sizeof("filename") - 1, 0)) || overwrite)
            obj -> sFilename = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "unparsed_uri", sizeof("unparsed_uri") - 1, 0)) || overwrite)
            obj -> sUnparsedUri = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "uri", sizeof("uri") - 1, 0)) || overwrite)
            obj -> sUri = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "path_info", sizeof("path_info") - 1, 0)) || overwrite)
            obj -> sPathInfo = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "query_info", sizeof("query_info") - 1, 0)) || overwrite)
            obj -> sQueryInfo = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "language", sizeof("language") - 1, 0)) || overwrite)
            obj -> sLanguage = (char *)ep_pstrdup(obj->pPool,((char *)epxs_sv2_PV((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
        if ((tmpsv = hv_fetch((HV *)item, "cookies", sizeof("cookies") - 1, 0)) || overwrite)
            obj -> pCookies = (HV *)SvREFCNT_inc(((HV *)epxs_sv2_HVREF((tmpsv && *tmpsv?*tmpsv:&PL_sv_undef))));
   ; }
...
CODE:
    ...
    if (initializer) {
        ...
        if (SvTYPE(tmpsv) == SVt_PVHV || SvTYPE(tmpsv) == SVt_PVMG)  
            Embperl__Req__Param_new_init (aTHX_ cobj, tmpsv, 1) ;
